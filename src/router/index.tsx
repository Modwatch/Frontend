...
export default class Routes extends Component<StoreProps, { fading: boolean, timeout: boolean }> {
  state = {
    fading: false,
    timeout: false
  }
  ...
  routeChange = e => {
    ...
    if(e.previous && e.previous.indexOf("/u/") === 0 && e.current.key === "modlist" && e.current.props.matches.username === e.previous.slice(3, e.current.props.matches.username.length + 3)) {
      return;
    }
  ...
  render() {
    return (
      <div class={`router-wrapper ${this.state.fading || this.state.timeout ? "fading" : ""}`}>
        <Router onChange={this.routeChange}>
          <AsyncRoute
            key={"home"}
            path="/"
            {...this.props as RouteProps}
            getComponent={() => this.wrappedImport(import("./home").then(c => c.default))}
          />
          <AsyncRoute
            key="modlist"
            path="/u/:username/:filetype?"
            {...this.props as RouteProps}
            getComponent={() => this.wrappedImport(import("./modlist").then(c => {
              return c.default;
            }))}
          />
          <AsyncRoute key="posts" path="/posts" {...this.props as RouteProps} getComponent={() => this.wrappedImport(import("./posts/index").then(c => c.default))} />
          <AsyncRoute key="post" path="/post/:title" {...this.props as RouteProps} getComponent={this.importPost} />
